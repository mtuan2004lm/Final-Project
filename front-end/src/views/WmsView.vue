<template>
    <div class="dashboard-container">
      ...
      <div class="main-content">
        <header><h1>PHÒNG QUẢN LÝ KHO HÀNG (WMS)</h1></header>
        <div class="content-body">
          <div class="card list-card">
              <h3>📦 Danh sách hàng chờ đóng gói & xuất kho</h3>
              <table class="data-table">
                  ...
                  <tbody>
                      <tr v-for="order in orders" :key="order.id">
                          ...
                          <td><span class="badge status-approved">{{ order.status }}</span></td>
                          <td>
                            <button @click="packOrder(order.id)" class="btn-action">Đóng gói & Chuyển Vận tải (TMS)</button>
                          </td>
                      </tr>
                      <tr v-if="orders.length === 0">
                          <td colspan="6" style="text-align: center; color: #7f8c8d; padding: 20px;">Kho trống - Chưa có đơn cần xử lý.</td>
                      </tr>
                  </tbody>
              </table>
          </div>
  ...
  const fetchOrders = async () => {
    try {
      ...
      console.error("Lỗi tải dữ liệu phòng WMS");
    }
  };
  ...
  const packOrder = async (orderId) => {
    try {
      ...
      alert("Đã hoàn tất đóng kho và bàn giao sang bộ phận Đội xe (TMS)!");
      fetchOrders();
    } catch (error) {
  ...
  <style scoped>
  /* Đồng bộ CSS với form tổng của OMS ở trên */
  ...
  .badge { padding: 5px 10px; background: #fff3cd; color: #856404; border-radius: 15px; font-size: 11px; font-weight: bold; }
  .btn-action { padding: 6px 12px; background: #27ae60; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; }
