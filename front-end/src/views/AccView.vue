     ...
     <div class="main-content">
        ...
        <div class="summary-cards-grid">
           <div class="summary-card revenue">
              ...
              <div class="card-info">
                 <p class="card-label">Customer Receivables (Collected)</p>
                 <h2 class="card-value">{{ summary.collectedCustomerRevenue }} USD</h2>
                 <small>Projected total revenue: {{ summary.totalCustomerRevenue }} USD</small>
              </div>
           </div>
           ...
           <div class="summary-card cost">
              <div class="card-icon">⛽</div>
              <div class="card-info">
                 <p class="card-label">Total Cost of E-POD for Drivers</p>
                 <h2 class="card-value">-{{ summary.totalEpodCost }} USD</h2>
                 <small>BOT: {{ summary.totalBotFee }} USD | Fuel: {{ summary.totalFuelFee }} USD</small>
              </div>
           </div>
           ...
           <div class="summary-card profit" :class="{ 'positive': summary.netProfit >= 0, 'negative': summary.netProfit < 0 }">
              <div class="card-icon">📈</div>
              <div class="card-info">
                 <p class="card-label">Actual Net Profit</p>
                 <h2 class="card-value">{{ summary.netProfit }} USD</h2>
                 <small>Actual profit margin</small>
              </div>
           </div>
        </div>

        <div class="card table-card">
           <h3>📥 General Ledger of Documents and Interdepartmental Cash Flow Separation</h3>
           ...
           <table class="data-table">
              <thead>
                 <tr>
                    ...
                    <th>Customer Payments (Receipts)</th>
                    <th>E-POD Price (Payment)</th>
                    <th>Warehouse/Vehicle Operations</th>
                    <th>Delivery Documents</th>
                    <th>Income/Expenditure Status</th>
                    <th>Action</th>
                 </tr>
              </thead>
              <tbody>
                 <tr v-for="order in orders" :key="order.id">
                    ...
                    <td>
                       ...
                       <span class="product-sub-txt">📦 Item: {{ order.product_name }} (Qty: {{ order.quantity }})</span>
                    </td>
                    <td>
                    ...
                    <td>
                       <div v-if="order.bot_fee > 0 || order.fuel_fee > 0" class="epod-breakdown">
                          <span class="money-out">- Total: {{ order.bot_fee + order.fuel_fee }} USD</span>
                          <small>• BOT fees: {{ order.bot_fee }} USD</small>
                          <small>• Fuel: {{ order.fuel_fee }} USD</small>
                       </div>
                       <span v-else class="text-muted">No issues have arisen yet.</span>
                    </td>
                    <td>
                       <span class="dept-badge">🏬 Warehouse: {{ order.warehouse_location }}</span>
                       <span class="dept-badge">🚚 Route: {{ order.delivery_route }}</span>
                    </td>
                    <td>
                       <div v-if="order.pod_image">
                          <span class="badge-success">✓ POD has been submitted.</span>
                          <span class="text-notes" v-if="order.driver_notes">📝 Note: "{{ order.driver_notes }}"</span>
                       </div>
                       <span v-else class="badge-pending">⏳ Not yet</span>
                    </td>
                    <td>
                       <span v-if="order.status === 'DONE'" class="status-tag status-done">💸 Settlement Completed</span>
                       <span v-else-if="order.payment_status === 'PAID'" class="status-tag status-paid">💰 Payment has been collected from the customer.</span>
                       <span v-else-if="order.payment_status === 'PENDING'" class="status-tag status-pending">⏳ Waiting for Fund Approval</span>
                       <span v-else class="status-tag status-unpaid">❌ Not yet paid</span>
                    </td>
                    <td>
                       <button
                       ...
                       >
                          {{ order.status === 'DELIVERED' ? 'E-POD Payment Approval' : 'Approve Customer Payment' }}
                       </button>
                       <span v-else class="text-locked">🔒The ledger has been closed.</span>
                    </td>
                 </tr>
                 <tr v-if="orders.length === 0">
                    <td colspan="8" style="text-align: center; color: #7f8c8d; padding: 30px;">Currently, there are no cash flow documents that need processing.</td>
                 </tr>
              </tbody>
           </table>
        </div>
...
export default {
   ...
   methods: {
      ...
      async approveOrderPayment(orderId) {
         if (!confirm(`Confirm approval of cash flow flow and close the ledger for the voucher. #${orderId}?`)) return;
         try {
            const response = await axios.put(`http://localhost:3000/api/orders/acc/${orderId}/approve`);
            alert(response.data.message || 'Cash flow approval successful!');
            this.fetchFinancialData(); // Cập nhật lại số liệu tức thì
         } catch (err) {
            alert('Error during the approval process: ' + err.message);
         }
      },
      logout() {
         localStorage.clear();
