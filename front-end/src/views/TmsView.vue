<template>
    <div class="dashboard-container">
      ...
      <div class="main-content">
        <header><h1>PHÒNG ĐIỀU PHỐI VẬN TẢI (TMS)</h1></header>
        <div class="content-body">
          <div class="card list-card">
              <h3>🚚 Danh sách đơn hàng đang trên đường vận chuyển</h3>
              <table class="data-table">
                  ...
                  <tbody>
                      <tr v-for="order in orders" :key="order.id">
                          ...
                          <td><span class="badge status-packed">{{ order.status }}</span></td>
                          <td>
                            <button @click="deliverOrder(order.id)" class="btn-action">Xác nhận giao thành công</button>
                          </td>
                      </tr>
                      <tr v-if="orders.length === 0">
                          <td colspan="6" style="text-align: center; color: #7f8c8d; padding: 20px;">Hiện tại không có chuyến xe nào đang chạy.</td>
                      </tr>
                  </tbody>
              </table>
          </div>
  ...
  const fetchOrders = async () => {
    try {
      const res = await axios.get('http://localhost:3000/api/orders/tms');
      orders.value = res.data;
    } catch (error) {
      console.error("Lỗi tải dữ liệu phòng TMS");
    }
  };
  ...
  const deliverOrder = async (orderId) => {
    try {
      ...
      alert("Hàng đã giao tới tay khách! Chuyển chứng từ về phòng lưu trữ (Docs).");
      fetchOrders();
    } catch (error) {
      alert("Xác nhận giao hàng lỗi!");
    }
  };
  ...
  .badge { padding: 5px 10px; background: #e2f0fd; color: #0066cc; border-radius: 15px; font-size: 11px; font-weight: bold; }
  .btn-action { padding: 6px 12px; background: #e67e22; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; }
